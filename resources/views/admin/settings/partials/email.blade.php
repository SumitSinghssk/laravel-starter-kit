@php
    ...
    $canEdit = auth()
        ->user()
        ->can('admin.settings.email.update');
    $value = fn (string $key) => old($key, $mail[$key]);
    $field = 'h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-xs transition outline-none focus:border-blue-400 focus:ring-3 focus:ring-blue-500/20 disabled:bg-slate-50 disabled:text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:disabled:bg-slate-800/60';
    $label = 'mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-200';
    $presets = collect(MailSettings::PRESETS)
        ->map(fn ($p) => ['host' => $p['host'], 'port' => $p['port'], 'encryption' => $p['encryption'], 'note' => $p['note']])
        ->all();
@endphp
...
<div class="space-y-5">
            ...
            async test() {
                ...
                try {
                    const { data } = await window.axios.post(
                        {{ Js::from(route('admin.settings.email.test')) }},
                        form,
                    )
                    this.result = data
                } catch (error) {
                    ...
                    if (response?.status === 429) {
                        this.result = {
                            ok: false,
                            message:
                                'Too many tests in a row. Wait a minute and try again.',
                        }
    ...
    >
        ...
        <x-admin.card
            title="Email (SMTP)"
            text="How the website sends email: contact form alerts, password resets and notifications. Works with any provider."
            icon="mail"
        >
            @if ($canEdit)
                <x-slot:actions>
            ...
            <fieldset @disabled(! $canEdit) class="space-y-6">
                ...
                >
                    <span class="flex items-start gap-3">
                        ...
                        <span>
                            <span class="block text-sm font-semibold text-slate-900 dark:text-white">
                                Send the website's email through this SMTP server
                            </span>
                            <span class="mt-0.5 block text-xs text-slate-500 dark:text-slate-400" x-show="enabled">
                                On. Every email the site sends uses the settings below.
                            </span>
                            <span class="mt-0.5 block text-xs text-slate-500 dark:text-slate-400" x-show="! enabled">
                                Off. The site uses the server's own mail setup (
                                <span class="font-mono">{{ MailSettings::fallbackMailer() }}</span>
                                ), which usually doesn't deliver real email.
                            </span>
                        </span>
                    </span>
                    ...
                    <span
                        class="relative mt-1 h-6 w-11 shrink-0 rounded-full bg-slate-300 transition peer-checked:bg-emerald-500 peer-focus-visible:ring-3 peer-focus-visible:ring-blue-500/30 dark:bg-slate-600"
                        aria-hidden="true"
                    >
                        <span
                ...
                <div>
                    ...
                    />
                    <p
                        class="mt-2 flex items-start gap-2 rounded-lg bg-blue-50/70 px-3 py-2 text-xs text-blue-800 dark:bg-blue-500/10 dark:text-blue-200"
                    >
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <button
                                    ...
                                    class="absolute inset-y-0 right-0 flex w-10 cursor-pointer items-center justify-center text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
                            ...
                            @if ($mail['has_password'])
                                <p class="mt-1.5 flex items-center justify-between gap-3 text-xs text-slate-500 dark:text-slate-400">
                                    <span class="flex items-center gap-1.5">
                                        <x-admin.icon name="lock" class="h-3.5 w-3.5" />
                                    ...
                                    <label class="flex cursor-pointer items-center gap-1.5 text-red-600 dark:text-red-400">
                                        <input
                                            ...
                                            value="1"
                                            x-model="removePassword"
                                            class="h-3.5 w-3.5 rounded border-slate-300 text-red-600"
                                        />
                                        Remove
                                    </label>
                                </p>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="{{ \App\Helpers\Settings::appName() }}"
                                maxlength="120"
                                class="{{ $field }}"
                            />
                        ...
                        <div class="sm:col-span-2">
                            <label for="mail-reply" class="{{ $label }}">
                                Reply-to
        ...
        @if ($canEdit)
            <x-admin.card title="Send a test email" text="Uses the settings above, even before you save them." icon="send">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start">
                    <div class="min-w-0 flex-1">
                        ...
                        <input
                            ...
                            type="email"
                            x-model="testEmail"
                            x-on:keydown.enter.prevent="test()"
                ...
                <p class="mt-3 flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400" x-show="testing" x-cloak>
                    ...
                    Connecting to
                    <span class="font-mono" x-text="host + ':' + port"></span>
                ...
                <template x-if="result">
                    ...
                    >
                        ...
                        <p class="mt-1 pl-6 text-xs opacity-75" x-show="result.ok && ! enabled">
                            It works. Turn on the switch above and save to use it for the website's email.
                        </p>
                    </div>
                </template>
            </x-admin.card>
